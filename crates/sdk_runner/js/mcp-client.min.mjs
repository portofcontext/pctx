...
// src/registry.ts
var MCPServerRegistry = class _MCPServerRegistry {
  constructor() {
    this.configs = /* @__PURE__ */ new Map();
  }
  /** Returns the singleton instance */
  static getInstance() {
    if (!_MCPServerRegistry.instance) {
      _MCPServerRegistry.instance = new _MCPServerRegistry();
    }
    return _MCPServerRegistry.instance;
  }
...
var REGISTRY = MCPServerRegistry.getInstance();
...
function registerMCP(cfg) {
  ...
  if (existing !== void 0) {
    throw new Error(
      `MCP Server with name "${existing.name}" is already registered, you cannot register two MCP servers with the same name`
    );
  }
  REGISTRY.add(cfg);
}
async function callMCPTool(call) {
  const mcpCfg = REGISTRY.get(call.name);
  if (mcpCfg === void 0) {
    throw new Error(`MCP Server with name "${call.name}" does not exist`);
  }
  const client = new Client({
    name: "ptx-client",
    version: "0.0.1"
  });
  const transport = new StreamableHTTPClientTransport(mcpCfg.url);
  ...
  if (toolRes.isError) {
    throw Error(`Tool call "${call.name}.${call.tool}" failed: ${toolRes}`);
  } else if (toolRes.structuredContent === void 0) {
    throw Error(
      `Tool call "${call.name}.${call.tool}" returned no structured content`
    );
  }
  return toolRes.structuredContent;
}
export {
  REGISTRY,
  callMCPTool,
