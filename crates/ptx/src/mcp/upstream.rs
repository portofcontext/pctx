...
pub(crate) async fn fetch_upstream_tools(server: &ServerConfig) -> Result<UpstreamMcp> {
    ...
    // For now, we return an empty UpstreamMcp since we don't have the MCP protocol
    // implementation to parse the actual tool list response.
    // TODO: Implement MCP protocol client to parse ListTools response
    debug!(
        "Successfully connected to '{}', but tool discovery not yet implemented",
        server.name
    );

    Ok(UpstreamMcp {
        name: server.name.clone(),
        namespace: server.name.clone(),
        description: format!("MCP server at {}", server.url),
        url: server.url.clone(),
        tools: IndexMap::new(), // Empty for now - will be populated when MCP client is implemented
    })
}
