...
/// Represents an upstream MCP server with its tools and metadata
...
pub struct UpstreamMcp {
    ...
    pub url: Url,
    pub tools: IndexMap<String, UpstreamTool>,
    pub registration: serde_json::Value,
}

impl UpstreamMcp {
    /// Connect to an upstream MCP server and introspect its tools
    pub async fn from_server(server: &ServerConfig) -> Result<Self> {
...
/// Represents a tool from an upstream MCP server with TypeScript type information
#[derive(Clone, Serialize, Deserialize)]
...
impl UpstreamTool {
    /// Convert an MCP tool into a TypeScript-typed upstream tool
    pub fn from_tool(tool: rmcp::model::Tool) -> Result<Self> {
    ...
    pub fn fn_impl(&self, mcp_name: &str) -> String {
        format!(
            "{fn_sig} {{
